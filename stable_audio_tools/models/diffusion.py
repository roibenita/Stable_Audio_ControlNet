...
from ..csa.csa import ControlDiffusionTransformer, ControlNet
...
class ControledConditionedDiffusionModel(nn.Module):
    def __init__(self,
                *args,
                supports_cross_attention: bool = False,
                supports_input_concat: bool = False,
...
class ControledConditionedDiffusionModelWrapper(nn.Module):
    """
    A diffusion model that takes in conditioning
    """
    def __init__(
            self,
            model: ControledConditionedDiffusionModel,
            conditioner: MultiConditioner,
            io_channels,
            sample_rate,
            min_input_length: int,
    ...
    def get_conditioning_inputs(self, conditioning_tensors: tp.Dict[str, tp.Any], negative=False):
        ...
        if len(self.control_ids) > 0:
            ...
            for key in self.control_ids:
                control_signal_input = conditioning_tensors[key][0]
                control_signal.append(control_signal_input)
            
            control_signal = torch.cat(control_signal, dim=1)
...
class ControledDiTWrapper(ControledConditionedDiffusionModel):
    def __init__(
        self,
        *args,
        **kwargs
...
## The args is empty, the kwargs is the diffuson model conf
        self.model = ControlDiffusionTransformer(*args, **kwargs)
...
# class ControlNetDiTWrapper(ConditionedDiffusionModel):
#     def __init__(
#         self,
#         *args,
#         **kwargs
...
#         ### Roi: ###
#         self.model = ControlNet(*args, **kwargs)
#         ######
...
def create_controled_diffusion_cond_from_config(config: tp.Dict[str, tp.Any]):
### Here the config has the whole config settings
    ...
    ##### Roi: Doing the controlnet declaration and init: #####
    ...
    ## Roi: Chnaging the Wrapper for the controled case ##
    if diffusion_model_type == 'controled_dit':
        diffusion_model = ControledDiTWrapper(ControlNet_config,**diffusion_model_config)
    #####

    # ##TODO: Here I dont use a wrapper and call the controlnet. maybe should change it in the future.
    # if diffusion_model_type == 'controled_dit':
    #     ControlNet_model = ControlNetDiTWrapper(**ControlNet_model_config)
    ...
    extra_kwargs = {}
    if model_type == "controled_diffusion_cond":
        wrapper_fn = ControledConditionedDiffusionModelWrapper
        extra_kwargs["diffusion_objective"] = diffusion_objective
