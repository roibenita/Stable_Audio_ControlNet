...
class prepare_conditioned_audio(nn.Module):
    ...
    def __init__(self, encoder_model, pre_transform_io_channels, dim_in, model_sample_rate ):
        super().__init__()
        ...
        self.preprocess_conv = nn.Conv1d(self.io_channels, self.io_channels, 1, bias=False)
    ...
    def forward(
        ...
        with torch.no_grad():
            ...
            for layer in self.layers:
                x = torch.cat([x, control_reverse.pop()], dim=1)
        ...
        ### TODO: should i have this art here? (i think that yes - actually each component from
        ## the original model should appeard
...
class ControlNet(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.zero_convs = nn.ModuleList([self.make_zero_conv(dim_heads)])
        ## TODO: make sure you chose right the dimentions.
        ...
        self.prepare_conditioned_audio = prepare_conditioned_audio(self.encoder, io_channels, self.dim_in, self.sample_rate )
        ...
        for i in range(depth):
            ...
            self.zero_convs.append(self.make_zero_conv(dim_heads))
...
## TODO: check that guided_hint dim is like the x dim (i think project hint dont need to change something)
        info = {
            "hidden_states": [],
        }

        guided_hint = self.project_in(guided_hint)
        ...
        if self.rotary_pos_emb is not None:
            rotary_pos_emb = self.rotary_pos_emb.forward_from_seq_len(guided_hint.shape[1])
        else:
            rotary_pos_emb = None

        if self.use_sinusoidal_emb or self.use_abs_pos_emb:
            guided_hint = guided_hint + self.pos_emb(guided_hint)
