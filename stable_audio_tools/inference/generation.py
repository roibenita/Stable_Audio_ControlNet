...
# def generate_diffusion_cond_control(
#         model,
#         steps: int = 250,
#         cfg_scale=6,
...
#     #### After getting the control audio, We should make a signal with the same length: ###
...
def generate_diffusion_cond(
    ...
    # Roi: Prepare the audio:
    #pad crop, clamp, stereo, emncoder, finishing in 1,64,1024
    # if conditioning_inputs['control_signal'] is not None:
    #     control_audio = conditioning_inputs['control_signal']
    #     control_audio = control_audio[0]
    #     in_sr = sample_rate
    #     io_channels = model.io_channels
    ...
    #     # Prepare the initial audio for use by the model
    #     control_audio = prepare_audio_gen(control_audio, in_sr=in_sr, target_sr=model.sample_rate, target_length=audio_sample_size, target_channels=io_channels, device=device)
    ...
    #     # For latent models, encode the initial audio into latents
    #     if model.pretransform is not None:
    #         control_audio = model.pretransform.encode(control_audio)
...
### for intensity: 
    if conditioning_inputs['control_signal'] is not None:
        ...
        ### from here I changed intensity: ###
        # intensity_len = calc_intensity_length(audio_sample_size, frame_length = 2048, hop_length = 512)
        # chunk_intensity = control_audio.new_zeros([2, intensity_len])

        # # Copy the audio into the chunk
        # chunk_intensity[:, :control_audio.shape[1]] = control_audio[ : ]

        # control_audio_final = chunk_intensity.repeat(batch_size, 1, 1)
        # conditioning_inputs['control_signal'] = control_audio_final
        #### till here was the previous intensity calc ####
